...
class LedgerWriterControllerTest {
    ...
    @Test
    @DisplayName("Given the transaction is internal and balance is insufficient, return HTTP 400")
    void addTransactionFailsWhenInsufficientBalance(TestInfo testInfo) {
        // Given
        ...
        when(transaction.getToAccountNum()).thenReturn("0987654321");
        when(transaction.getAmount()).thenReturn(500);
        when(transaction.getRequestUuid()).thenReturn(testInfo.getDisplayName());
        doReturn(10).when(spyController).getAvailableBalance(TOKEN, AUTHED_ACCOUNT_NUM);
